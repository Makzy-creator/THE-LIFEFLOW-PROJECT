...
const DonationOptions: React.FC = () => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Trigger AI matching/search for recipients
      const matches = await AIMatchingService.findMatches({
        bloodType: donationForm.type === 'gift-match' ? 'O+' : user.bloodType || 'O+', // fallback
        location: donationForm.location || user.location || '',
        urgency: 'medium', // or get from form if available
        distance: 50 // default max distance in km
      });
      if (matches && matches.length > 0) {
        toast.success(`AI found ${matches.length} potential recipients!`);
      } else {
        toast('No immediate matches found, but your donation will be listed.');
      }
  ...
  return (
    <div className="space-y-8">
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to search for available recipients/donors now?"
